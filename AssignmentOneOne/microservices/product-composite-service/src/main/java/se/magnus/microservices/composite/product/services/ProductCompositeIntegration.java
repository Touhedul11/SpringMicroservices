...
public class ProductCompositeIntegration implements ProductService, RecommendationService, ReviewService {
    ...
    public ProductCompositeIntegration(
        ...
        @Value("${app.review-service.port}") int    reviewServicePort,
            ...
            @Value("${app.customers-service.port}") int    customersServicePort
    ) {
        ...
        customersServiceUrl      = "http://" + customersServiceHost + ":" + customersServicePort + "/customer?productId=";
    }
    ...
    public List<Customer> getCustomers(int productId) {
        ...
        try {
            String url = customersServiceUrl + productId;

            LOG.debug("Will call getCustomers API on URL: {}", url);
            List<Customer> customers= restTemplate.exchange(url, GET, null, new ParameterizedTypeReference<List<Customer>>() {}).getBody();

            LOG.debug("Found {} customers for a product with id: {}", customers.size(), productId);
            return customers;

        } catch (Exception ex) {
            LOG.warn("Got an exception while requesting customer, return zero customer: {}", ex.getMessage());
            return new ArrayList<>();
        }
    }
}
